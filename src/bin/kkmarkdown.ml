...
type env = { unsafe : bool; input_files : string list }

let parse_arg () =
  let unsafe = ref false in
  let input_files_rev = ref [] in
  let speclist = [ ("--unsafe", Arg.Set unsafe, "Enable unsafe mode") ] in
  Arg.parse speclist
...
|};
  { unsafe = !unsafe; input_files = List.rev !input_files_rev }

let get_input_from_stdin () = Util.get_input_from_channel stdin

let main { unsafe; input_files } =
